...
export interface DNSResult { A?:string[]; MX?:string[]; NS?:string[]; TXT?:string[]; CNAME?:string[]; }
...
export function classifyTarget(h:string):"hardened"|"demo"|"standard" {
  ...
  const demo=["vulnweb.com","dvwa","juice-shop","bwapp","metasploitable","127.0.0.1","webgoat","demo.testfire.net","hackazon","testphp","testasp","testhtml","testaspnet"];
...
export async function fetchLiveHeaders(hostname:string):Promise<LiveAuditResult> {
  try {
    ...
    const r=await fetch(`/api/audit?url=${encodeURIComponent(url)}`,{signal:AbortSignal.timeout(10000)});
...
export async function fetchFuzz(hostname:string,paths:string):Promise<{path:string,status:number}[]> {
  try {
    const r=await fetch(`/api/fuzz?url=${encodeURIComponent(hostname)}&paths=${encodeURIComponent(paths)}`,{signal:AbortSignal.timeout(20000)});
    const d=await r.json();
    return d.results||[];
  } catch { return[]; }
}

export async function fetchSqli(hostname:string):Promise<{vulnerable:boolean,payload:string,endpoint:string}> {
  try {
    const r=await fetch(`/api/sqli?url=${encodeURIComponent(hostname)}`,{signal:AbortSignal.timeout(10000)});
    const d=await r.json();
...
export function buildDNSFindings(dns:DNSResult,hostname:string):AuditFinding[] {
  ...
  const hasDMARC=(dns.TXT||[]).some((t:string)=>t.includes("v=DMARC1"));
...
export async function runAudit(
  ...
  if(tier!=="hardened" || profile==="fullPenTest"){
    ...
    const fuzzResults = await fetchFuzz(hostname, fuzzPaths);
    
    const hits = fuzzResults.filter(r => r.status === 200 || r.status === 401 || r.status === 403);
    ...
    hits.forEach(h => {
      ...
      if(h.path.includes(".env") || h.path.includes(".git")) { severity = "critical"; title = `Critical Data Exposure: ${h.path}`; }
      if(h.path.includes("admin") || h.path.includes("backup")) { severity = "high"; }
      ...
      fuzzFindings.push({
        ...
        evidence:`Real HTTP HEAD request to ${h.path} returned HTTP ${h.status}.`,
        ...
        cvss: severity==="critical"?9.1:severity==="high"?7.5:5.0, riskScore: severity==="critical"?9.1:severity==="high"?7.5:5.0,
        exploitPotential:"easy", affectedComponents:["Web Server Routing"], references:[], discoveredAt:new Date().toISOString(), confidence:"confirmed"
      });
    });
  ...
  if(tier==="demo") {
    ...
    const sqliResult = await fetchSqli(hostname);
    if(sqliResult) {
